...
import {
  LayoutDashboard, Users, BarChart3, BookOpen, Calendar,
  ClipboardList, Brain, FileText, Settings, ChevronLeft,
  ChevronRight, GraduationCap, Bell, Upload, UserCog, Activity,
} from "lucide-react";
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }}
                ...
                <span className="block text-[10px] text-muted-foreground -mt-1">Admin Portal</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          return (
            <Link key={item.path} to={item.path}
              className={cn(
                ...
                isActive ? "bg-primary/20 text-foreground" : "text-muted-foreground hover:bg-white/[0.06] hover:text-foreground"
              )}>
              {isActive && (
